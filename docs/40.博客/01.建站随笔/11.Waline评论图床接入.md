...
但接上自建 Cloudflare-ImgBed 图床之后，评论区直接支持**粘贴/选择图片自动上传**，体验完全不一样。更进一步，我还让它根据当前页面路径**自动归档到对应文件夹**，图床后台不会乱成一锅粥。
